...
$t05_siswarutin_add = NULL; // Initialize page object first

class ct05_siswarutin_add extends ct05_siswarutin {
	...
	// Page object name
	var $PageObjName = 't05_siswarutin_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t05_siswarutinlist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t05_siswarutinview.php")
...
// Create page object
if (!isset($t05_siswarutin_add)) $t05_siswarutin_add = new ct05_siswarutin_add();

// Page init
$t05_siswarutin_add->Page_Init();

// Page main
$t05_siswarutin_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t05_siswarutin_add->Page_Render();
?>
...
var CurrentForm = ft05_siswarutinadd = new ew_Form("ft05_siswarutinadd", "add");

// Validate form
ft05_siswarutinadd.Validate = function() {
...
ft05_siswarutinadd.Form_CustomValidate = 
...
ft05_siswarutinadd.ValidateRequired = true;
<?php } else { ?>
ft05_siswarutinadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft05_siswarutinadd.Lists["x_rutin_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t04_rutin"};
...
<?php if (!$t05_siswarutin_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t05_siswarutin_add->ShowPageHeader(); ?>
<?php
$t05_siswarutin_add->ShowMessage();
?>
<form name="ft05_siswarutinadd" id="ft05_siswarutinadd" class="<?php echo $t05_siswarutin_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t05_siswarutin_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t05_siswarutin_add->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($t05_siswarutin->getCurrentMasterTable() == "t03_siswa") { ?>
...
<?php if ($t05_siswarutin->siswa_id->Visible) { // siswa_id ?>
	<div id="r_siswa_id" class="form-group">
		<label id="elh_t05_siswarutin_siswa_id" for="x_siswa_id" class="col-sm-2 control-label ewLabel"><?php echo $t05_siswarutin->siswa_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t05_siswarutin->siswa_id->CellAttributes() ?>>
<?php if ($t05_siswarutin->siswa_id->getSessionValue() <> "") { ?>
<span id="el_t05_siswarutin_siswa_id">
...
<input type="hidden" id="x_siswa_id" name="x_siswa_id" value="<?php echo ew_HtmlEncode($t05_siswarutin->siswa_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_t05_siswarutin_siswa_id">
<input type="text" data-table="t05_siswarutin" data-field="x_siswa_id" name="x_siswa_id" id="x_siswa_id" size="30" placeholder="<?php echo ew_HtmlEncode($t05_siswarutin->siswa_id->getPlaceHolder()) ?>" value="<?php echo $t05_siswarutin->siswa_id->EditValue ?>"<?php echo $t05_siswarutin->siswa_id->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $t05_siswarutin->siswa_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_rutin_id"><?php echo (strval($t05_siswarutin->rutin_id->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $t05_siswarutin->rutin_id->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($t05_siswarutin->rutin_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_rutin_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="t05_siswarutin" data-field="x_rutin_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t05_siswarutin->rutin_id->DisplayValueSeparatorAttribute() ?>" name="x_rutin_id" id="x_rutin_id" value="<?php echo $t05_siswarutin->rutin_id->CurrentValue ?>"<?php echo $t05_siswarutin->rutin_id->EditAttributes() ?>>
<input type="hidden" name="s_x_rutin_id" id="s_x_rutin_id" value="<?php echo $t05_siswarutin->rutin_id->LookupFilterQuery() ?>">
</span>
<?php echo $t05_siswarutin->rutin_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php echo $t05_siswarutin->Nilai->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php
	if (in_array("t06_siswarutinbayar", explode(",", $t05_siswarutin->getCurrentDetailTable())) && $t06_siswarutinbayar->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t05_siswarutin_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft05_siswarutinadd.Init();
</script>
<?php
$t05_siswarutin_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t05_siswarutin_add->Page_Terminate();
?>
