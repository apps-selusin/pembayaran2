...
$t02_kelas_delete = NULL; // Initialize page object first

class ct02_kelas_delete extends ct02_kelas {
	...
	// Page object name
	var $PageObjName = 't02_kelas_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t02_kelaslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t02_kelas class, t02_kelasinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t02_kelaslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t02_kelas_delete)) $t02_kelas_delete = new ct02_kelas_delete();

// Page init
$t02_kelas_delete->Page_Init();

// Page main
$t02_kelas_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t02_kelas_delete->Page_Render();
?>
...
var CurrentForm = ft02_kelasdelete = new ew_Form("ft02_kelasdelete", "delete");

// Form_CustomValidate event
ft02_kelasdelete.Form_CustomValidate = 
...
ft02_kelasdelete.ValidateRequired = true;
<?php } else { ?>
ft02_kelasdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft02_kelasdelete.Lists["x_sekolah_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t01_sekolah"};
...
<?php $t02_kelas_delete->ShowPageHeader(); ?>
<?php
$t02_kelas_delete->ShowMessage();
?>
<form name="ft02_kelasdelete" id="ft02_kelasdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t02_kelas_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t02_kelas_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t02_kelas">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t02_kelas_delete->RecKeys as $key) { ?>
...
<?php if ($t02_kelas->sekolah_id->Visible) { // sekolah_id ?>
		<th><span id="elh_t02_kelas_sekolah_id" class="t02_kelas_sekolah_id"><?php echo $t02_kelas->sekolah_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t02_kelas->Nama->Visible) { // Nama ?>
		<th><span id="elh_t02_kelas_Nama" class="t02_kelas_Nama"><?php echo $t02_kelas->Nama->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t02_kelas_delete->RecCnt = 0;
$i = 0;
while (!$t02_kelas_delete->Recordset->EOF) {
	$t02_kelas_delete->RecCnt++;
	$t02_kelas_delete->RowCnt++;

	// Set row properties
	$t02_kelas->ResetAttrs();
	$t02_kelas->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t02_kelas_delete->LoadRowValues($t02_kelas_delete->Recordset);

	// Render row
	$t02_kelas_delete->RenderRow();
?>
	<tr<?php echo $t02_kelas->RowAttributes() ?>>
<?php if ($t02_kelas->sekolah_id->Visible) { // sekolah_id ?>
		<td<?php echo $t02_kelas->sekolah_id->CellAttributes() ?>>
<span id="el<?php echo $t02_kelas_delete->RowCnt ?>_t02_kelas_sekolah_id" class="t02_kelas_sekolah_id">
...
<?php } ?>
<?php if ($t02_kelas->Nama->Visible) { // Nama ?>
		<td<?php echo $t02_kelas->Nama->CellAttributes() ?>>
<span id="el<?php echo $t02_kelas_delete->RowCnt ?>_t02_kelas_Nama" class="t02_kelas_Nama">
...
<?php
	$t02_kelas_delete->Recordset->MoveNext();
}
$t02_kelas_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t02_kelas_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft02_kelasdelete.Init();
</script>
<?php
$t02_kelas_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t02_kelas_delete->Page_Terminate();
?>
