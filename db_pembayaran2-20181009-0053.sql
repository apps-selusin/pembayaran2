...
--
-- Host: localhost
-- Generation Time: Oct 08, 2018 at 07:53 PM
...
INSERT INTO `t06_siswarutinbayar` (`id`, `siswarutin_id`, `Bulan`, `Tahun`, `Bayar_Tgl`, `Bayar_Jumlah`) VALUES
(1, 1, 7, 2018, NULL, 0.00),
(2, 1, 8, 2018, NULL, 0.00),
(3, 1, 9, 2018, NULL, 0.00),
(4, 1, 10, 2018, NULL, 0.00),
(5, 1, 11, 2018, NULL, 0.00),
(6, 1, 12, 2018, NULL, 0.00),
(7, 1, 1, 2019, NULL, 0.00),
(8, 1, 2, 2019, NULL, 0.00),
(9, 1, 3, 2019, NULL, 0.00),
(10, 1, 4, 2019, NULL, 0.00),
(11, 1, 5, 2019, NULL, 0.00),
(12, 1, 6, 2019, NULL, 0.00),
(13, 2, 7, 2018, NULL, 0.00),
(14, 2, 8, 2018, NULL, 0.00),
(15, 2, 9, 2018, NULL, 0.00),
(16, 2, 10, 2018, NULL, 0.00),
(17, 2, 11, 2018, NULL, 0.00),
(18, 2, 12, 2018, NULL, 0.00),
(19, 2, 1, 2019, NULL, 0.00),
(20, 2, 2, 2019, NULL, 0.00),
(21, 2, 3, 2019, NULL, 0.00),
(22, 2, 4, 2019, NULL, 0.00),
(23, 2, 5, 2019, NULL, 0.00),
(24, 2, 6, 2019, NULL, 0.00);
...
--
-- Stand-in structure for view `v02_rutin`
-- (See below for the actual view)
--
CREATE TABLE `v02_rutin` (
...
--
-- Structure for view `v02_rutin`
--
DROP TABLE IF EXISTS `v02_rutin`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v02_rutin`  AS  select `c`.`tahunajaran_id` AS `tahunajaran_id`,`d`.`awal_bulan` AS `awal_bulan`,`d`.`awal_tahun` AS `awal_tahun`,`d`.`akhir_bulan` AS `akhir_bulan`,`d`.`akhir_tahun` AS `akhir_tahun`,`e`.`sekolah_id` AS `sekolah_id`,`f`.`Nama` AS `Sekolah`,`c`.`kelas_id` AS `kelas_id`,`e`.`Nama` AS `Kelas`,`b`.`siswa_id` AS `siswa_id`,`c`.`Nomor_Induk` AS `Nomor_Induk`,`c`.`Nama` AS `Nama`,`b`.`rutin_id` AS `rutin_id`,`g`.`Nama` AS `Pembayaran_Rutin`,`b`.`Nilai` AS `nilai`,`a`.`id` AS `id`,`a`.`siswarutin_id` AS `siswarutin_id`,`a`.`Bulan` AS `Bulan`,`a`.`Tahun` AS `Tahun`,`a`.`Bayar_Tgl` AS `Bayar_Tgl`,`a`.`Bayar_Jumlah` AS `Bayar_Jumlah` from ((((((`t06_siswarutinbayar` `a` left join `t05_siswarutin` `b` on((`a`.`siswarutin_id` = `b`.`id`))) left join `t03_siswa` `c` on((`b`.`siswa_id` = `c`.`id`))) left join `t00_tahunajaran` `d` on((`d`.`id` = `c`.`tahunajaran_id`))) left join `t02_kelas` `e` on((`e`.`id` = `c`.`kelas_id`))) left join `t01_sekolah` `f` on((`f`.`id` = `e`.`sekolah_id`))) left join `t04_rutin` `g` on((`g`.`id` = `b`.`rutin_id`))) ;
