...
<?php include_once "t02_kelasgridcls.php" ?>
...
class ct01_sekolah_list extends ct01_sekolah {
	...
	function __construct() {
		...
		$this->AddUrl = "t01_sekolahadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			// Process auto fill for detail table 't02_kelas'
			if (@$_POST["grid"] == "ft02_kelasgrid") {
				if (!isset($GLOBALS["t02_kelas_grid"])) $GLOBALS["t02_kelas_grid"] = new ct02_kelas_grid;
				$GLOBALS["t02_kelas_grid"]->Page_Init();
				$this->Page_Terminate();
				exit();
			}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "ft01_sekolahlistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ft01_sekolahlistsrch", $filters);
	...
	function SetupListOptions() {
		...
		// "detail_t02_kelas"
		$item = &$this->ListOptions->Add("detail_t02_kelas");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 't02_kelas') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_t02_kelas"
		$oListOpt = &$this->ListOptions->Items["detail_t02_kelas"];
		if ($Security->AllowList(CurrentProjectID() . 't02_kelas')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("t02_kelas", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("t02_kelaslist.php?" . EW_TABLE_SHOW_MASTER . "=t01_sekolah&fk_id=" . urlencode(strval($this->id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["t02_kelas_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 't02_kelas')) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=t02_kelas")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "t02_kelas";
			}
			if ($GLOBALS["t02_kelas_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 't02_kelas')) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=t02_kelas")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "t02_kelas";
			}
			if ($GLOBALS["t02_kelas_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 't02_kelas')) {
				$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=t02_kelas")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
				if ($DetailCopyTblVar <> "") $DetailCopyTblVar .= ",";
				$DetailCopyTblVar .= "t02_kelas";
			}
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_t02_kelas");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=t02_kelas");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["t02_kelas"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["t02_kelas"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 't02_kelas') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "t02_kelas";
		}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ft01_sekolahlistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_t01_sekolah\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_t01_sekolah',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ft01_sekolahlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
var CurrentSearchForm = ft01_sekolahlistsrch = new ew_Form("ft01_sekolahlistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($t01_sekolah_list->SearchOptions->Visible()) { ?>
<?php $t01_sekolah_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($t01_sekolah_list->FilterOptions->Visible()) { ?>
<?php $t01_sekolah_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php if ($t01_sekolah->Export == "") { ?>
...
<?php
	...
	if ($t01_sekolah_list->AuditTrailOnSearch && $t01_sekolah_list->Command == "search" && !$t01_sekolah_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $t01_sekolah_list->getSessionWhere();
		$t01_sekolah_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$t01_sekolah_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($t01_sekolah->Export == "" && $t01_sekolah->CurrentAction == "") { ?>
<form name="ft01_sekolahlistsrch" id="ft01_sekolahlistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($t01_sekolah_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ft01_sekolahlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="t01_sekolah">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($t01_sekolah_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($t01_sekolah_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $t01_sekolah_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($t01_sekolah_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($t01_sekolah_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($t01_sekolah_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($t01_sekolah_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php if ($t01_sekolah->Nomor_Induk->Visible) { // Nomor_Induk ?>
	<?php if ($t01_sekolah->SortUrl($t01_sekolah->Nomor_Induk) == "") { ?>
		<th data-name="Nomor_Induk"><div id="elh_t01_sekolah_Nomor_Induk" class="t01_sekolah_Nomor_Induk"><div class="ewTableHeaderCaption"><?php echo $t01_sekolah->Nomor_Induk->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Nomor_Induk"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t01_sekolah->SortUrl($t01_sekolah->Nomor_Induk) ?>',2);"><div id="elh_t01_sekolah_Nomor_Induk" class="t01_sekolah_Nomor_Induk">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t01_sekolah->Nomor_Induk->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t01_sekolah->Nomor_Induk->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t01_sekolah->Nomor_Induk->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t01_sekolah->Nama->Visible) { // Nama ?>
	...
	<?php } else { ?>
		<th data-name="Nama"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t01_sekolah->SortUrl($t01_sekolah->Nama) ?>',2);"><div id="elh_t01_sekolah_Nama" class="t01_sekolah_Nama">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t01_sekolah->Nama->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t01_sekolah->Nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t01_sekolah->Nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($t01_sekolah->Alamat->Visible) { // Alamat ?>
	...
	<?php } else { ?>
		<th data-name="Alamat"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t01_sekolah->SortUrl($t01_sekolah->Alamat) ?>',2);"><div id="elh_t01_sekolah_Alamat" class="t01_sekolah_Alamat">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $t01_sekolah->Alamat->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($t01_sekolah->Alamat->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($t01_sekolah->Alamat->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($t01_sekolah->Nomor_Induk->Visible) { // Nomor_Induk ?>
		<td data-name="Nomor_Induk"<?php echo $t01_sekolah->Nomor_Induk->CellAttributes() ?>>
<span id="el<?php echo $t01_sekolah_list->RowCnt ?>_t01_sekolah_Nomor_Induk" class="t01_sekolah_Nomor_Induk">
...
<?php if ($t01_sekolah->Export == "") { ?>
<script type="text/javascript">
ft01_sekolahlistsrch.FilterList = <?php echo $t01_sekolah_list->GetFilterList() ?>;
ft01_sekolahlistsrch.Init();
